$(document).ready(function() {
	 $('.link_form_popup').click(function(e) {
		  e.preventDefault();
		  document.getElementById('wrap_form_popup').style.display='block';
		  $('#wrap_form_popup').reveal({
...
function getBodyScrollTop()
{
   return self.pageYOffset || (document.documentElement && document.documentElement.scrollTop) || (document.body && document.body.scrollTop);
	...
	$(function() {
		...
		var i = getBodyScrollTop();
		var pages = window.innerHeight - 45;		
		i = i + pages;
...
$('.tabs li').click(function(){ 
	var offset_top = $(".wrap").offset();
	var number = $(this).find('a').attr('href');
	number = number.charAt( (number.length - 1) );
	var offset = number * 76 - 140;
	var wrapper_width = $(document).width();
	if(wrapper_width < 750){
		$('html, body').scrollTop(offset_top.top + offset);
	}
});	
...
//Navigation Scrolling
	$(function() {
		$('.nav a, .nav li a, #home-center a, a.move, #logo a').bind('click',function(event){
		var $anchor = $(this);
...
//Elements Fading
	...
	$('.element_fade_in').each(function () {
		$(this).appear(function() {
			$(this).delay(150).animate({opacity:1,right:"0px"},100);
		});	
	});
	
	$('.element_task_top').each(function () {
		$(this).appear(function() {
		  $(this).delay(150).animate({opacity:1,top:"0px"},1000);
		});	
	});
	$('.element_task_bottom').each(function () {
		$(this).appear(function() {
		  $(this).delay(150).animate({opacity:1,bottom:"0px"},1000);
		});	
	});
...
//Google Map					
	var latlng = new google.maps.LatLng(55.77517528,37.611913);
	var settings = {
		zoom: 16,
		center: new google.maps.LatLng(55.77517528,37.611913), mapTypeId: google.maps.MapTypeId.ROADMAP,
	...
	var contentString = '<div id="content">'+
		...
		'<div id="bodyContent">'+
		'<p>Москва, улица Делегатская, 5, стр 1</p>'+
		'</div>'+
		'</div>';
	...
	var companyPos = new google.maps.LatLng(55.77517528,37.611913);
